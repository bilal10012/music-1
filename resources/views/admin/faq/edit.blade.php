...
@section('content')
    <div class="content">
        <h2 class="content-heading">FAQ #{{ $faq_detail->id }} ({{ $faq_detail->question }}) <a
                href="{{ route('admin.faq.index') }}" class="btn btn-alt-primary pull-right">Back</a></h2>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-12">
                                    <input type="text" class="form-control" id="name" name="question"
                                        ...
                                        value="{{ old('question') ? old('question') : $faq_detail->question }}">
                                    @if ($errors->has('title'))
                                        <div class="text-danger ml-5 font-weight-bold">{{ $errors->first('question') }}
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="col-12">
                                    <input type="text" class="form-control" id="name" name="answer"
                                        placeholder="answer"
                                        value="{{ old('answer') ? old('answer') : $faq_detail->answer }}">
                                    @if ($errors->has('answer'))
                                        <div class="text-danger ml-5 font-weight-bold">{{ $errors->first('answer') }}</div>
                                    @endif
                                </div>

                                <label class="col-12">Display on Section</label>
                                <div class="col-12">
                                    <select class="form-control" name="section">
                                        <option value="">Select Section</option>
                                        <option value="1"
                                            {{ old('section', $faq_detail->section) == 1 ? 'selected' : '' }}>Section 1
                                        </option>
                                        <option value="2"
                                            {{ old('section', $faq_detail->section) == 2 ? 'selected' : '' }}>Section 2
                                        </option>
                                        <option value="3"
                                            {{ old('section', $faq_detail->section) == 3 ? 'selected' : '' }}>Section 3
                                        </option>
                                    </select>
                                    @if ($errors->has('section'))
                                        <div class="text-danger ml-5 font-weight-bold">{{ $errors->first('section') }}
                                        </div>
                                    @endif
                                </div>
